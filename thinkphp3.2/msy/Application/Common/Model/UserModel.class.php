...
class UserModel extends Model {
    ...
    protected $_validate = array(
        ...
        array('name','accountIsExist','用户名不存在！',self::EXISTS_VALIDATE,'function',self::MODEL_UPDATE),
        array('pass_word','require','请填写密码！'),
        array('pass_word','checkPwd','密码格式不正确',self::EXISTS_VALIDATE,'function',self::MODEL_BOTH),
        array('re_password','pass_word','密码与确认密码不一致',self::EXISTS_VALIDATE,'confirm'),
        array('mobile_phone','require','请填写手机号码！'),
        array('mobile_phone','isMobile','手机号格式不正确',self::EXISTS_VALIDATE,'function',self::MODEL_BOTH),
        array('mobile_phone','','此手机号已被注册，请更换手机号码！',self::EXISTS_VALIDATE,'unique',self::MODEL_INSERT),
//        array('mobile_phone','isReservedMobilePhone','不是预留手机号码！',0,'function',self::MODEL_UPDATE),//总出错
        array('captcha','require','请填写验证码！',self::MUST_VALIDATE),
    );
