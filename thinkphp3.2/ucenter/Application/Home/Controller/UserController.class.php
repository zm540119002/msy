...
class UserController extends CommonController {
    private $_ucenterSessionId = '';
    private $_ucenterSessionName = 'UCENTER_SESSION_ID';
    ...
    public function register(){
        if (IS_POST) {
            print_r($this->ajaxReturn(123));
//            print_r($_COOKIE);
            exit;
            $redis = new \Redis();
            $redis->connect(C('REDIS_HOST'),C('REDIS_PORT'));
            $this->_ucenterSessionId = session_id();
            $redis->set($this->_ucenterSessionName,$this->_ucenterSessionId);
            print_r($redis->get($this->_ucenterSessionName));
            setcookie($this->_ucenterSessionName,$this->_ucenterSessionId,time()+3600);
            exit;
            $this->_register();
        } else {
            $this->display();
