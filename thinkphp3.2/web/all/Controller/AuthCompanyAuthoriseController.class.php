<?php
namespace web\all\Controller;

use web\all\Cache\CompanyCache;

/**机构认证的控制器基类
 */
class AuthCompanyAuthoriseController extends AuthUserController{
    protected $companyAuthoriseUrl = 'Home/CompanyAuthorise/index';    //机构认证URL

    public function __construct(){
        parent::__construct();
        CompanyCache::remove($this->user['id']);
        $this->company = CompanyCache::get($this->user['id']);
        //机构认证
        $dueTime = C('DEFAULT_DUE_TIME');   //到期时间
        if(!$this->company || $this->company['auth_status'] == 0 ||
            ($this->company['auth_status'] == 1 &&  time() - $this->company['create_time']  > $dueTime) ){
            $this->error(C('ERROR_COMPANY_AUTHORISE_REMIND'),U($this->companyAuthoriseUrl));
        }
    }
}