...
class GoodsController extends BaseController {
    ...
    public function goodsManage(){
        if(IS_POST){
            ...
            //所有商品分类
            $this->allCategoryList = D('GoodCategory')->selectGoodsCategory();
            $this->display();
        }
    }
    ...
    public function goodsList(){
        ...
        if(IS_POST){
            ...
            $field = array(
                ...
                'g.price','g.special_price','g.group_price','g.inventory','g.sort',
    ...
    public function goodsEdit(){
        ...
        if(IS_POST){
            if( isset($_POST['main_img']) && $_POST['main_img'] ){
                $_POST['main_img'] = $this->moveImgFromTemp(C('MYMS_GOODS_MAIN_IMG'),basename($_POST['main_img']));
            }
            if( isset($_POST['detail_img']) && $_POST['detail_img'] ){
                ...
                foreach ($detailArr as $item) {
                    if($item){
                        $tempArr[] = $this->moveImgFromTemp(C('MYMS_GOODS_DETAIL_IMG'),basename($item));
                    }
                }
            ...
            if(isset($_POST['goodsId']) && intval($_POST['goodsId'])){//修改
                ...
                if($goodsInfo['detail_img']){
                    ...
                    if(empty($oldImgArr)){
                        $oldImgArr[] = $goodsInfo['detail_img'];
                    }
    ...
    public function goodsDel(){
        ...
        if(isset($_POST['goodsId']) && intval($_POST['goodsId'])){
            ...
            $goodsInfo = $model->selectGoods($where);
            //删除商品图片
            if(!empty($goodsInfo[0]['main_img'])){
                $mainImg = $goodsInfo[0]['main_img'];
                $this->delImg($mainImg);
            }
            if(!empty($goodsInfo[0]['detail_img'])){
                $detailImg = $goodsInfo[0]['detail_img'];
                $detailImgArr = implode(',', $detailImg);
                if(empty($detailImgArr)){
                    $detailImgArr = $detailImg;
                }
                $this->delImg($detailImgArr);
            }
            $res = $model->delGoods();
        }
        $this->ajaxReturn($res);
        ...
        if(IS_POST){
            ...
            if(isset($_POST['projectId']) && $_POST['projectId']){
                ...
                $field = array(
                    ...
                    'g.sort','g.specification','g.price','g.group_price','g.inventory','g.main_img','g.detail_img','g.create_time',
                    'g.intro',
                );
                $join = array(
    ...
    public function commonImageEdit(){
        ...
        if(IS_POST) {
            if (isset($_POST['common_img']) && $_POST['common_img']) {
                ...
                foreach ($detailArr as $item) {
                    if ($item) {
                        $tempArr[] = $this->moveImgFromTemp(C('MYMS_GOODS_COMMON_IMG'), basename($item));
                    }
                }
