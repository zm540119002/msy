$(function(){
            ...
            if (msg.status == 1) {
                ...
                if(!$('.cart').find('li').length){
                    ...
                    $('.cart').find('li').each(function(){
                        ...
                        if(buyType == msg.info.info.buy_type && id == msg.info.info.id && msg.info.info.type){
    ...
    $('.shopping_count').on('blur',function(){
        ...
        $.post(MODULE + '/Cart/addCart', {foreignId: foreignId,goodsNum:goodsNum,buyType:buyType,type:type}, function (msg) {
            if(msg.status == 0){
                dialog.error(msg.info);
            }
    ...
    $('.edit_delete').on('click',function(){
        ...
        layer.open({
            ...
            yes:function(index){
                ...
                var type = _thisLi.data('type');
                $.post(MODULE + '/Cart/delCart', {cartId: cartId,foreignId:foreignId,buyType:buyType,type:type}, function (msg) {
                    if(msg.status == 1){
                        _thisLi.remove();
    ...
    $('body').on('click','.plus',function(){
        ...
        $.post(MODULE + '/Cart/addCart', {foreignId: foreignId,goodsNum:shopNum,buyType:buyType,type:type}, function (msg) {
            if(msg.status == 0){
                dialog.error(msg.info);
            }
            if(msg.status == 1){
    ...
    $('body').on('click','.reduce',function(){
        ...
        if(shopNum==0){
            ...
            $.post(MODULE + '/Cart/delCart', {foreignId:foreignId,type:type,buyType:buyType,cartId:cartId}, function (msg) {
                if(msg.status == 1){
                    TotalPrice();
                }
                if(msg.status == 0){
