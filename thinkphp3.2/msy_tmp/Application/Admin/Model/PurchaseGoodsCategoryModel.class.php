...
class PurchaseGoodsCategoryModel extends Model {
    ...
    //删除分类
    public function delGoodsCategory($ids=[]){
        $where = array(
            'id'=>array('in',$ids),
        );
    ...
    public function selectGoodsCategory($where=[],$field=[],$join=[]){
        ...
        $_field = array(
            'gc.id','gc.name','gc.status','gc.level','gc.parent_id','gc.parent_id_1','gc.parent_id_2','gc.sort',
        );
        $_join = array(
        );
        $list = $this
