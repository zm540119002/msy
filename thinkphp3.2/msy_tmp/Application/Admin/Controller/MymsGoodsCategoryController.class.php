...
class MymsGoodsCategoryController extends BaseController {
    /**商品分类-管理
     */
    public function goodsCategoryManage(){
        $model = D('MymsGoodCategory');
        if(IS_POST){
    ...
    public function goodsCategoryEdit(){
        ...
        if(IS_POST){
            if( isset($_POST['img']) && $_POST['img'] ){
                $_POST['img'] = $this->moveImgFromTemp(C('MYMS_GOODS_CATEGORY_IMG'),basename($_POST['img']));
            }
