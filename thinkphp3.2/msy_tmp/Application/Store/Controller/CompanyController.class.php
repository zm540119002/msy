...
class CompanyController extends AuthUserController {
    ...
    public function authInfo(){
        if(IS_POST){
            ...
            $info = M('Company') -> where($where) -> field('name,organizajc')->find();
    ...
    public function uploadImg(){
        ...
        //如果有图片先删除再上传
        $field = M('Company') ->where($where) ->getField($field);
    ...
    public function authCommit(){
        if(IS_POST) {
            ...
            $data = I('post.');
            $result = D('Company') -> where($where) ->save($data);
            if($result){
                show(1,"提交资料完成");
            }else{
                show(0,"无法提交资料");
