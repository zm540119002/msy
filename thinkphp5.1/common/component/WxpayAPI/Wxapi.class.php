...
//			$baseUrl = urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].$_SERVER['QUERY_STRING']);
    ...
    public function GetOpenidFromMp($code)
    {
        ...
        if(C('WEIXINCONFIG')['CURL_PROXY_HOST'] != "0.0.0.0"
            && C('WEIXINCONFIG')['CURL_PROXY_PORT'] != 0){
            curl_setopt($ch,CURLOPT_PROXY, C('WEIXINCONFIG')['CURL_PROXY_HOST']);
            curl_setopt($ch,CURLOPT_PROXYPORT, C('WEIXINCONFIG')['CURL_PROXY_PORT']);
        }
        //运行curl，结果以jason形式返回
        $res = curl_exec($ch);
        curl_close($ch);
