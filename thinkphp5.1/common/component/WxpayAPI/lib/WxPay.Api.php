...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(C('WX_CONFIG')['NOTIFY_URL']);//异步通知url
		}
		
		$inputObj->SetAppid(C('WX_CONFIG')['APPID']);//公众账号ID
		$inputObj->SetMch_id(C('WX_CONFIG')['MCHID']);//商户号
	...
	{
		//如果不需要上报数据
		if(C('WX_CONFIG')['REPORT_LEVENL']== 0){
			return;
		} 
		//如果仅失败上报
		if(C('WX_CONFIG')['REPORT_LEVENL'] == 1 &&
...
//		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,2);//严格校验
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, C('WX_CONFIG')['SSLCERT_PATH']);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, C('WX_CONFIG')['SSLKEY_PATH']);
		}
		//post提交方式
