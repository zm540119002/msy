	...
	<section class="slider_banner">
		<div class="swiper-container swiper-container-horizontal swiper-container-ios">
			<div class="swiper-wrapper">
				...
				<div class="swiper-slide swiper-slide-active">
					<img src="http://mch.meishangyun.com/uploads/<?php echo htmlentities($main_img); ?>" alt="" class="common_default_img">
				</div>
				<?php endforeach; endif; else: echo "" ;endif; ?>
			</div>
		</div>
	</section>
	<section class="content-padding content_main wy_list" style="background:url(http://mch.meishangyun.com/uploads/<?php echo htmlentities($info['background_img']); ?>);">
		<div class="">
			<div class="top_b_text wk_notice separation">
				<p class="more-text">
				<span class="intro_title">[品牌定制须知]</span>
				品牌定制利润为王！维雅生物药妆精选优质项目产品任你选，帮助用户开启自主品牌可持续发展经营！用户在线选购产品并支付品牌定制单，其他全部交给维雅品牌定制服务团队。
				</p>
			</div>
		...
		<ul class="goods_list list">
			<?php if(!(empty($goodsList) || (($goodsList instanceof \think\Collection || $goodsList instanceof \think\Paginator ) && $goodsList->isEmpty()))): if(is_array($goodsList) || $goodsList instanceof \think\Collection || $goodsList instanceof \think\Paginator): $i = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?>
			<li data-id="<?php echo htmlentities($info['id']); ?>" data-buy_type="1" id="li-id-<?php echo htmlentities($key); ?>">
				<a class="p_img" href="<?php echo url('Goods/detail',['id'=>$info['id']]); ?>">
					...
					<img class="left" src="http://mch.meishangyun.com/static/common/img/default/no_pic_100.jpg" alt="">
					<?php else: ?>
					<img class="left" src="http://mch.meishangyun.com/uploads/<?php echo htmlentities($info['thumb_img']); ?>" alt="">
					<?php endif; ?>
				</a>
				<div class="list_r">
					<p class="text-intro"><?php echo htmlentities($info['headline']); ?></p>
					<!--<div>单品规格<span><?php echo htmlentities($info['specification']); ?></span></div>-->
					<div>
						<span class="red">￥<price><?php echo htmlentities($info['bulk_price']); ?></price></span>
					</div>
					<div class="columns_flex l-r-sides">
						<div>起订数量：<span><?php echo htmlentities($info['minimum_order_quantity']); ?><?php echo htmlentities(getUnit($info['purchase_unit'])); ?></span>
						</div>
						<!--<a href="javascript:void(0);" class="sample_purchase">样品购买
							<input type="hidden" class="" value="<?php echo htmlentities($info['minimum_sample_quantity']); ?>">
						</a>-->
					</div>
					...
					<div class="quantity_wrapper selected-number">
						...
						<input type="hidden" class="minimum_order_quantity" value="<?php echo htmlentities($info['minimum_order_quantity']); ?>">
						<input type="hidden" class="increase_quantity" value="<?php echo htmlentities($info['increase_quantity']); ?>">
